...
def _reference_pre(residual, fn, hc_scale, hc_base, sinkhorn_repeat=20):
...
def _reference_post(x, residual, post_mix, comb_mix):
...
def test_telechat4_mhc_triton_matches_reference(num_tokens, implementation):
    ...
    expected_pre = _reference_pre(residual, fn, hc_scale, hc_base)
    ...
    if implementation == "split":
        ...
        assert comb_mix_for_post.data_ptr() == comb_mix.data_ptr()
    expected_post = _reference_post(x, residual, post_mix, comb_mix_for_post)
...
def test_telechat4_mhc_triton_dispatch_layout_only_for_eager_extend(monkeypatch):
    if not hasattr(torch, "npu") or not torch.npu.is_available():
        pytest.skip("Ascend NPU is required")

    from sglang.srt.models import telechat4 as telechat4_model

    monkeypatch.setattr(telechat4_model, "_NPU_MHC_BACKEND", "triton")
    ...
    with get_forward().scoped(is_extend_in_batch=False):
        graph_comb = telechat4_model.mhc_pre(
            residual, fn, hc_scale, hc_base, 1e-6, 1e-6, 1e-6, 2.0, 20, 1, 8
        )[1]
    with get_forward().scoped(is_extend_in_batch=True):
        eager_extend_comb = telechat4_model.mhc_pre(
            residual, fn, hc_scale, hc_base, 1e-6, 1e-6, 1e-6, 2.0, 20, 1, 8
        )[1]
...
def test_telechat4_mhc_ascendc_dispatch_matches_reference(monkeypatch):
    if not hasattr(torch, "npu") or not torch.npu.is_available():
        pytest.skip("Ascend NPU is required")
    ...
    assert telechat4_model._NPU_MHC_BACKEND == "ascendc"
    ...
    expected_post = _reference_post(x, residual, actual_pre[0], comb_for_post)
